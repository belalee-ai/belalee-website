...
我在某跨境电商平台欧洲区负责广告投放的那段时间，做的事情说白了就是这三件：在 eCPM 竞价框架里找到效率最优解，在 ROAS 约束下做预算的动态分配，以及管理几百条素材从生到死的全过程。AI 广告产品做的也是这三件事，只是执行方式从人变成了模型。
...
在某电商平台投放 Facebook 广告时，我面对的典型场景是这样的：一条素材上线 6 小时，花了 200 美元，点击率 1.8%，转化率 0.6%，ROAS 0.7。这条素材要不要继续跑？
...
在某电商平台，我主导过从 Auto Bid 向 ROAS 出价的转型。这件事听起来很简单——把出价方式从"系统自动出价"切换成"按目标 ROAS 出价"就行了。但真正的难点不在切换本身，在于**切换之后怎么建立一套持续有效的调控机制**。
...
在该平台的投放体系里，我们同时在跑的素材经常有上百条。管理这些素材不是"好的多花钱、差的关掉"就完了。
...
**头部素材（ROAS > 120%，占比约 10%）：** 不是简单加预算，而是用"复制广告组"的方式放量。直接加预算会触发学习期重置，但复制一个新的广告组、用相同素材和定向，相当于在不打扰原有广告组的情况下开了第二条通道。单条头部素材我们做到过日花费 8,000 美元以上、ROAS 稳定在 115% 以上。
